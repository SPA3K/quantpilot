...
<div class="tabs">
  <div class="tab active" onclick="switchTab('ml')">🧠 ML因子选股</div>
  <div class="tab" onclick="switchTab('build')">🔧 DIY策略</div>
...
function switchTab(n){
  document.querySelectorAll('.tab').forEach(function(t,i){
    t.classList.toggle('active',['ml','build','presets','results','catalog'][i]===n);
  });
  ['ml','build','presets','results','catalog'].forEach(function(id){
    document.getElementById('tab-'+id).style.display=id===n?'':'none';
  });
}

function runMLRecommend(){
  if(!ML_DATA||!ML_DATA.length){alert('暂无ML数据');return}
  var latest=ML_DATA[ML_DATA.length-1];
  var preds=Object.entries(latest.predictions).map(function(kv){
    return{code:kv[0],name:kv[1].name,l0:kv[1].l0,l1:kv[1].l1,l3:kv[1].l3,fusion:kv[1].fusion,ret:kv[1].actual_return}
  });
  var top10=preds.slice().sort(function(a,b){return b.fusion-a.fusion}).slice(0,10);
  ...
  top10.forEach(function(s,i){
    h+='<tr style="border-bottom:1px solid var(--bd)">';
    h+='<td style="padding:8px 10px;font-weight:700;color:var(--a2)">'+(i+1)+'</td>';
    h+='<td style="padding:8px 10px;font-family:monospace">'+s.code+'</td>';
    h+='<td style="padding:8px 10px;font-weight:600">'+s.name+'</td>';
    h+='<td style="padding:8px 10px;text-align:right;font-family:monospace;color:'+(s.l0>=0?'var(--gn)':'var(--rd)')+'">'+(s.l0>=0?'+':'')+s.l0.toFixed(3)+'</td>';
    h+='<td style="padding:8px 10px;text-align:right;font-family:monospace;color:'+(s.l1>=0?'var(--gn)':'var(--rd)')+'">'+(s.l1>=0?'+':'')+s.l1.toFixed(3)+'</td>';
